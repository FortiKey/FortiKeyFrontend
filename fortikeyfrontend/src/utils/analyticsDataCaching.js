...
export const getCacheStats = () => {
    ...
    return {
        ...
        endpointStats,
        oldestEntry: keys.length ? new Date(Math.min(...keys.map(k => dataCache[k].timestamp))).toISOString() : null,
        newestEntry: keys.length ? new Date(Math.max(...keys.map(k => dataCache[k].timestamp))).toISOString() : null
    };
};